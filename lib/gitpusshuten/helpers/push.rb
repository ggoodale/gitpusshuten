    ...
    module Push
      ...
      def confirm_remote!
        unless git.has_remote?(environment.name)
          ...
          GitPusshuTen::Log.error "To do that, you must run: " + "gitpusshuten setup remote for #{environment.name}".color(:yellow)
      ...
      def add_remote?
        GitPusshuTen::Log.message "Would you like to run #{"gitpusshuten setup remote for #{environment.name}".color(:yellow)} the above command now?"
        choose do |menu|
          menu.prompt = ''
